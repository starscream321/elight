...
export class EffectsRunnerService {
  ...
  private getEffect(effectName: string): EffectFn | undefined {
    return this.effectsService.getEffectByName(effectName) as
      | EffectFn
      | undefined;
  }
  ...
  async start(
    ...
    const loop = async () => {
      if (!this.isRunning || activeRunId !== this.runId || !this.currentEffect)
        return;
      ...
      try {
        ...
        for (let u = 0; u < this.totalUniverses; u++) {
          ...
          sends.push(this.artnetService.sendPacket(target, u, this.ipAddress));
        }
        ...
        if (this.isRunning && activeRunId === this.runId) {
          ...
          this.ticker = setTimeout(
            loop,
            Math.max(0, this.frameIntervalMs - elapsed),
          );
        }
      } catch (error) {
